import Link from 'next/link'

type LogoProps = {
  className?: string
  showText?: boolean
}

const Logo: React.FC<LogoProps> = ({ className = '', showText = true }) => {
  return (
    <Link
      href='/'
      className={`inline-flex items-center gap-2 sm:gap-2.5 whitespace-nowrap text-inherit ${className}`}>
      <svg
        xmlns='http://www.w3.org/2000/svg'
        viewBox='0 0 32 32'
        className='h-7 w-7 sm:h-8 sm:w-8 lg:h-9 lg:w-9 shrink-0 fill-primary'
        aria-hidden='true'>
        <path d='M0 23.63l2.703 4.672c0.552 1.094 1.667 1.781 2.885 1.781h17.943l-3.724-6.453zM32 23.661c0-0.641-0.193-1.245-0.516-1.75l-10.516-18.276c-0.557-1.057-1.656-1.719-2.854-1.719h-5.557l16.24 28.135 2.563-4.432c0.5-0.849 0.641-1.224 0.641-1.958zM17.161 19.047l-7.255-12.568-7.26 12.568z' />
      </svg>
      {showText && (
        <span className='text-xl sm:text-2xl lg:text-3xl font-semibold'>
          G Project
        </span>
      )}
      <span className='sr-only'>G Project</span>
    </Link>
  )
}

export default Logo
